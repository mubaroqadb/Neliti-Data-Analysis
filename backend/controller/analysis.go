...
// GetRecommendations handler untuk mendapatkan rekomendasi metode analisis
func GetRecommendations(w http.ResponseWriter, r *http.Request, projectIDStr string) {
	...
	if req.UploadID != "" {
		...
		// Ambil upload terbaru dari project
	...
	logAudit(userID, "recommend", "analyses", "Generated recommendations for project: "+project.Title, r)
...
// ProcessAnalysis handler untuk memproses analisis dengan metode yang dipilih
...
// GetAnalysisResults handler untuk mendapatkan hasil analisis
...
// RefineAnalysis handler untuk refine analisis berdasarkan feedback
...
func processMethod(method string, project model.Project, upload model.Upload) model.MethodResult {
	...
	if err == nil {
		...
		result.Interpretation = fmt.Sprintf("Analisis %s telah dilakukan. Silakan interpretasikan hasil berdasarkan output statistik.", method)
	}
	
	result.Conclusion = "Kesimpulan akan dihasilkan berdasarkan interpretasi hasil analisis."
