...
Konfigurasi GoCroot memanfaatkan variabel lingkungan untuk menyimpan rahasia dan parameter koneksi. Varian yang lazim meliputi MONGOSTRING, PUBLICKEY, PRIVATEKEY, dan daftar origin CORS. Semua ini ditangani di paket config, yang kemudian diekspor sebagai variabel bernilai ke lapisan lain (controller, url). Pada produksi, penggunaan .env untuk pengembangan dan secrets pada platform CI/CD (misalnya GitHub Actions) adalah pendekatan yang lazim, dengan pembedaan jelas antara “development” dan “production”[^1][^3][^16].
